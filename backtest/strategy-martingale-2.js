...
function calculateNextPosition(ps, price, newProfit, settings, timestamp) {
    ...
    const needNewPosition = lastPosition.direction === 'BUY' ? lastPosition.openPrice > price : lastPosition.openPrice < price;

    if (needNewPosition) {
        const newPositionAmount = positions[positions.length - 1].amount + 1;
