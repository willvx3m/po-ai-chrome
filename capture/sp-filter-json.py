...
def process_file(file_path, destination_dir):
    ...
    if destination_dir:
        start_label = result[0]["time_label"].replace(':', '.')
        end_label = result[-1]["time_label"].replace(':', '.')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process json files to adjust time and price.')
    ...
    for file_path in all_files:
        result = json.load(open(file_path, 'r'))
        if "_" not in file_path.split('/')[-1]:
            continue

        process_file(file_path, args.destination_dir)